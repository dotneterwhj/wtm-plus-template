			...
			<el-form ref="formRef" :model="state.vmModel" label-width="100px">
				<el-row v-if="state.vmModel.DpType == 'User'">
					<el-col :xs="24"  :lg="12"  class="mb20">
						<el-form-item ref="Entity_UserCode_FormItem" prop="Entity.UserCode"
							...
							<el-input v-model="state.vmModel.Entity.UserCode" clearable disabled></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row v-else>
				...
				<el-row>
					<el-col :xs="24"  :lg="12"  class="mb20">
						<el-form-item ref="Entity_TableName_FormItem" prop="Entity.TableName"
							...
							<el-select v-model="state.vmModel.Entity.TableName" @change="modelChange"  disabled>
					...
					<el-col :xs="24"  :lg="12"  class="mb20">
						<el-form-item ref="Entity_IsAll_FormItem" prop="IsAll"
							...
							<el-select v-model="state.vmModel.IsAll"  disabled>
				...
				<el-row v-if="state.vmModel.IsAll==false">
					<el-col :xs="24"  :lg="24"  class="mb20">
						<el-form-item ref="Entity_SelectedItemsID_FormItem" prop="Entity.SelectedItemsID"
							:label="$t('message._admin.dp.vm.SelectedItemsID')">
							<el-select v-model="state.vmModel.SelectedItemsID" multiple  disabled>
								<el-option v-for="item in state.AllPrivilegeTableNames" :key="item.Value" :value="item.Value"
									:label="item.Text">{{
										item.Text
									}}</el-option>
							</el-select>
...
const onSubmit = () => {
	...
	formRef.value?.validate((valid: boolean, fields: any) => {
		if (valid) {
			dataPrivilegeApi().edit(state.vmModel).then(() => {
